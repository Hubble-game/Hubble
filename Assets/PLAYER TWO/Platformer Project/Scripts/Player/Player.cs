...
namespace PLAYERTWO.PlatformerProject
{
	...
	public class Player : Entity<Player>
	{
		...
		public void ApplySpeedJumpBoost(float speedMultiplier, float jumpMultiplier, float duration)
		{
			...
			// If a previous boost is active, stop it and restore values first.
			...
			m_statBoostCoroutine = StartCoroutine(SpeedJumpBoostRoutine(speedMultiplier, jumpMultiplier, duration));
		}

		protected Coroutine m_statBoostCoroutine;

		protected System.Collections.IEnumerator SpeedJumpBoostRoutine(float speedMultiplier, float jumpMultiplier, float duration)
		{
			var s = stats.current;
			// Cache original values
			var originalTopSpeed = s.topSpeed;
			var originalRunningTopSpeed = s.runningTopSpeed;
			var originalMaxJump = s.maxJumpHeight;
			var originalMinJump = s.minJumpHeight;

			// Apply multipliers
			s.topSpeed = originalTopSpeed * speedMultiplier;
			s.runningTopSpeed = originalRunningTopSpeed * speedMultiplier;
			s.maxJumpHeight = originalMaxJump * jumpMultiplier;
			s.minJumpHeight = originalMinJump * jumpMultiplier;

			// Wait duration
			yield return new WaitForSeconds(duration);

			// Restore original values
			s.topSpeed = originalTopSpeed;
			s.runningTopSpeed = originalRunningTopSpeed;
			s.maxJumpHeight = originalMaxJump;
